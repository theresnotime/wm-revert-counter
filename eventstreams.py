...
from urllib3.util.response import httplib
...
class EventStreams(GeneratorWrapper):
    ...
    def __repr__(self) -> str:
        ...
        if kwargs["timeout"] == config.socket_timeout:
            kwargs.pop("timeout")
        return "{}({})".format(
            self.__class__.__name__, ", ".join(f"{k}={v!r}" for k, v in kwargs.items())
        )
    ...
    def url(self):
        ...
        if self._streams is None:
            raise NotImplementedError(
                "No streams specified for class {}".format(self.__class__.__name__)
            )
    ...
    def set_maximum_items(self, value: int) -> None:
        ...
        if value is not None:
            ...
            debug(
                "{}: Set limit (maximum_items) to {}.".format(
                    self.__class__.__name__, self._total
                )
            )
    ...
    def generator(self):
        ...
        debug(
            "{}: Stopped iterating due to exceeding item limit.".format(
